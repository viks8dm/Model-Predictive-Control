...
public:
    ...
    void operator()(ADvector& fg, const ADvector& vars) {
        ...
        for (unsigned int i = 0; i < N; i++) {
            fg[0] += WT_CTE * CppAD::pow(vars[cte_start + i] - REF_CTE, 2);
            fg[0] += WT_EPSI * CppAD::pow(vars[epsi_start + i] - REF_EPSI, 2);
            fg[0] += WT_V * CppAD::pow(vars[v_start + i] - REF_V, 2);
        }

        // Minimize the use of actuators.
        for (unsigned int i = 0; i < N - 1; i++) {
            fg[0] += WT_DELTA * CppAD::pow(vars[delta_start + i], 2);
            fg[0] += WT_A * CppAD::pow(vars[a_start + i], 2);
        }

        // Minimize the value gap between sequential actuations.
        for (unsigned int i = 0; i < N - 2; i++) {
            fg[0] += WT_DELTA_DIFF * CppAD::pow(vars[delta_start + i + 1] - vars[delta_start + i], 2);
            fg[0] += WT_A_DIFF * CppAD::pow(vars[a_start + i + 1] - vars[a_start + i], 2);
        }
...
Result MPC::Solve(Eigen::VectorXd x0, Eigen::VectorXd coeffs) {
    ...
    // initialize parameters from incoming state vector
    double x = x0[0];
    double y = x0[1];
    double psi = x0[2];
    double v = x0[3];
    ...
    size_t n_vars = N * NUM_VARIABLES + (N - 1) * NUM_ACTUATORS;
    // Number of constraints
    size_t n_constraints = N * NUM_VARIABLES;
    ...
    // Lower and upper limits for x (state vector)
    ...
    for (int i = 0; i < delta_start; i++) {
        vars_lowerbound[i] = -MAX_LIMITS;
        vars_upperbound[i] = MAX_LIMITS;
    }
    ...
    for (int i = delta_start; i < a_start; i++) {
        vars_lowerbound[i] = -STEER_LIMIT_RAD;
        vars_upperbound[i] = STEER_LIMIT_RAD;
    }

    // Acceleration/decceleration upper and lower limits.
    // NOTE: Feel free to change this to something else.
    for (int i = a_start; i < n_vars; i++) {
        vars_lowerbound[i] = -ACTUATOR_LIMIT;
